...
fn run_once(deq: &mut VecDeque<(i64, i128)>, input: &[(i64, i128)]) {
    for (key, _) in input {
...
fn mix(vals: &[i128], times: usize) -> i128 {
    let input: Vec<(i64, i128)> = vals
        .iter()
        .enumerate()
        .map(|(i, v)| (i as i64, *v))
        .collect();
    let mut deq: VecDeque<(i64, i128)> = input.iter().copied().collect();
...
pub fn solve(input: &str, verify_expected: bool, output: bool) -> Result<Duration> {
    let mut input: Vec<i128> = input.lines().map(|s| s.parse().unwrap()).collect();
